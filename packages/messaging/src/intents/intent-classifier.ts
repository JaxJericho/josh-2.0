import type { ConversationSession, IntentClassification } from "./intent-types";
...
export function classifyIntent(
  ...
  if (isHelpKeyword(normalizedForKeywordMatch)) {
    return buildClassification("HELP", 1);
  }
...
function isHelpKeyword(normalizedMessage: string): boolean {
  return HELP_KEYWORDS.has(normalizedMessage);
}
