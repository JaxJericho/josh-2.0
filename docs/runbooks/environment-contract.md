...
| `ADMIN_SESSION_SECRET` | Yes (admin routes), No (non-admin runtime) | `app/lib/admin-session.ts` signed admin session cookie integrity (`josh_admin_session`) | High-entropy secret, minimum 32 chars; no whitespace | Local: `.env.local` for admin auth testing. Staging: Vercel staging env. Production: Vercel production env. | Secret. Rotate if exposed; rotating invalidates existing admin sessions. |
...
| Name | Required | Used by | Format / constraints | Where set (local / staging / production) | Security / rotation notes |
|---|---|---|---|---|---|
| `SUPABASE_URL` | Yes | Next API routes (`app/api/webhooks/twilio/status/route.ts`, `app/api/cron/reconcile-outbound/route.ts`, `app/api/stripe/webhook/route.ts`), Supabase Edge Functions | Valid Supabase REST base URL, no trailing spaces | Local: `.env.local`/shell. Staging: Vercel staging env and Supabase staging function secret. Production: Vercel production env and Supabase production function secret. | Treat as sensitive operational config. Keep per-env separation. |
...
| `ADMIN_OPS_SECRET` | Yes (staging), No (local), No (production) | `supabase/functions/admin-set-entitlements` secret-header auth (`x-admin-secret`) | High-entropy random token; minimum 32 chars; no whitespace | Local: optional `.env.local` for local verification. Staging: Supabase staging function secret and operator shell for verification curls. Production: not used until dashboard bearer-auth launch. | Secret. Rotate immediately if exposed; never log value. |
...
| Name | Required | Used by | Format / constraints | Where set (local / staging / production) | Security / rotation notes |
|---|---|---|---|---|---|
| `TWILIO_ACCOUNT_SID` | Yes | `supabase/functions/twilio-outbound-runner` send API calls; reconcile route Twilio lookup (`app/api/cron/reconcile-outbound/route.ts`) | Must start with `AC` and match account for auth token | Local: shell for function serve. Staging: Supabase staging function secret + Vercel staging env (for reconcile route). Production: Supabase production function secret + Vercel production env (if/when reconcile enabled). | Sensitive account identifier; rotate only via account migration. |
| `TWILIO_AUTH_TOKEN` | Yes | Twilio webhook signature verification and outbound API auth; reconcile route Twilio lookup (`app/api/cron/reconcile-outbound/route.ts`) | Twilio auth token string | Local: `.env.local`/shell and local functions. Staging: Vercel staging env + Supabase staging secrets. Production: Vercel production env + Supabase production secrets. | Secret. Rotate immediately if leaked. |
...
| Name | Required | Used by | Format / constraints | Where set (local / staging / production) | Security / rotation notes |
|---|---|---|---|---|---|
| `CRON_SECRET` | Yes | Cron route bearer auth (`app/api/cron/run-outbound/route.ts`, `app/api/cron/reconcile-outbound/route.ts`) | Random high-entropy token; no whitespace | Local: `.env.local` for local cron endpoint tests. Staging: Vercel staging env. Production: Vercel production env. | Secret. Rotate with cron config update in lockstep. |
...
| `QSTASH_TOKEN` | Yes (onboarding scheduler integration) | `app/lib/qstash.ts` delayed publish client | Upstash QStash token string from QStash console | Local: `.env.local` for local verification. Staging: Vercel staging env. Production: Vercel production env. | Secret. Rotate in Upstash, then update all environments. |
| `QSTASH_CURRENT_SIGNING_KEY` | Yes (onboarding scheduler integration) | `app/lib/qstash.ts` inbound request signature verification | Current Upstash signing key string | Local: `.env.local` for local verification. Staging: Vercel staging env. Production: Vercel production env. | Secret. Rotate with dual-key window and keep in sync with QStash keyset. |
| `QSTASH_NEXT_SIGNING_KEY` | Yes (onboarding scheduler integration) | `app/lib/qstash.ts` key-rotation-safe inbound signature verification | Next Upstash signing key string | Local: `.env.local` for local verification. Staging: Vercel staging env. Production: Vercel production env. | Secret. Required to avoid verification breakage during key rotation. |
| `ONBOARDING_SCHEDULING_DISABLED` | No (runtime guardrail) | `app/lib/qstash.ts`, `supabase/functions/_shared/engines/onboarding-engine.ts` scheduler short-circuit for onboarding-only disable | Boolean-like flag: `1`/`true` disables publish, `0`/`false` enables publish | Local: optional `.env.local`. Staging: set in Vercel staging env and Supabase staging function secrets only during controlled disable windows. Production: leave unset unless incident response requires temporary onboarding scheduling disable. | Non-secret. Must be scoped to onboarding scheduling only; reset after incident mitigation. |
...
| `SENTRY_RELEASE` | Yes (staging/prod observability), No (local) | `sentry.server.config.ts`, `sentry.edge.config.ts`, `sentry.client.config.ts` release tag + issue grouping | Deployment identifier string (git SHA or semantic version) | Local: optional `.env.local`. Staging: Vercel staging env. Production: Vercel production env. | Non-secret. Must be updated every deploy for accurate regressions and release health. |
