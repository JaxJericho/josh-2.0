...
function errorSummary(status: number): Summary {
  return { ok: false, runner_status: status, expiredCount: 0 };
}
...
async function handleCron(request: Request): Promise<Response> {
  ...
  const handler = "api/cron/invitation-expiry-runner";
  ...
  const expiredCount = clampNonNegativeInt(upstreamJson?.expiredCount);
...
function resolveRunnerUrl(): string | null {
  ...
  try {
    const url = new URL("/functions/v1/invitation-expiry-runner", supabaseUrl);
    return url.toString();
  } catch {
    return null;
  }
